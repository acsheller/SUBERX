...
def linear_scheduler(initial_value: float):
    def func(progress: float) -> float:
        return initial_value * progress
    return func

class CombinedCallback(BaseCallback):
    def __init__(self, save_freq=2000, log_freq=100, save_path="./tmp/models/", name_prefix="rl_model", verbose=0):
...
if __name__ == "__main__":
    ...
    model = A2C(
        ...
        learning_rate=linear_scheduler(args.learning_rate),
        tensorboard_log=save_path,
        gamma=args.gamma,
        ent_coef=0.01,
    )

    combined_callback = CombinedCallback(save_freq=1000, log_freq=100, save_path=save_path, name_prefix="rl_model", verbose=1)
